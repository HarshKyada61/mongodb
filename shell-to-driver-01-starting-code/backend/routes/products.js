...
router.get('/:id', (req, res, next) => {
  db.getDb().db().collection('products').findOne({_id: new objectId(req.params.id)}).then(productDoc => {
    productDoc.price = productDoc.price.toString();
...
db.getDb().db().collection('products').insertOne(newProduct).then(results => {
          console.log(results);
          res.status(201).json({ message: 'Product added', productId: results.insertedId });
        }).catch(err=> {
          console.log(err);
          res.status(500).json({ message: 'error occured'});
        });
